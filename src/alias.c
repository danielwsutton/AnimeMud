...
void substitute_alias(DESCRIPTOR_DATA * d, char *argument)
{
	...
	if (strlen(ch->prefix) + strlen(argument) > MAX_INPUT_LENGTH)
	    ...
	    sprintf(prefix, "%s `7%s``", ch->prefix, argument);
	    argument = prefix;
	}
    }
...
void do_alias(CHAR_DATA * ch, char *argument)
{
    ...
    smash_tilde(argument);	/*EE960617 */

    argument = one_argument(argument, arg);

    if (IS_AFFECTED(ch, AFF_CHARM) && ch->master != NULL) {
	send_to_char("Lucky for you that you're charmed.\n\r", ch);
	return;
    }
	...
	|| !str_cmp("password", arg)) {
	send_to_char("Sorry, that word is reserved.\n\r", ch);
	return;
    }

    /* FIX HERE FOR PFILE CORRUPTING STUPIDITY */
    if (strchr(arg, ' ') || strchr(arg, '"') || strchr(arg, '\'')) {
	send_to_char
	    ("You can't use space, tick, or quote marks in an alias.\n\r",
	     ch);
	return;
    }
	...
	|| !str_prefix(argument, "password")
	|| !str_prefix(argument, "title")) {
	...
	if (!str_cmp(arg, rch->pcdata->alias[pos])) {	/* redefine an alias */
	    ...
	    sprintf(buf, "%s is now realiased to '`7%s``'.\n\r", arg,
		    argument);
	    send_to_char(buf, ch);
	    return;
	}
    ...
    rch->pcdata->alias_sub[pos] = str_dup(argument);
    sprintf(buf, "%s is now aliased to '`7%s``'.\n\r", arg, argument);
    send_to_char(buf, ch);
}
