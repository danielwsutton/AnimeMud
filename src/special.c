...
DECLARE_SPEC_FUN(spec_cabbit);	/* Suzuran */
DECLARE_SPEC_FUN(spec_happosai);	/* Suzuran */
...
const struct spec_type spec_table[] = {
    ...
    {"spec_cabbit", spec_cabbit},	/* Suzuran */
    {"spec_happosai", spec_happosai},	/* Suzuran */
    {NULL, NULL}
};
...
bool spec_nasty(CHAR_DATA * ch)
{
    ...
    if (victim->in_room != ch->in_room) /* make sure they must be in the room to steal */
	return FALSE;
...
bool spec_cast_adept(CHAR_DATA * ch)
{
    ...
    case 6:
	act("$n utters the words '`@judicandus eugzagz``'.", ch, NULL,
	    NULL, TO_ROOM);
...
bool spec_janitor(CHAR_DATA * ch)
{
	...
	if (trash->item_type == ITEM_DRINK_CON
	    ...
	    act
		("`6$n grumbles and says '`^People today can't pick up their own trash.`6'``",
		 ch, NULL, NULL, TO_ROOM);
...
bool spec_mayor(CHAR_DATA * ch)
{
    ...
    case 'e':
	act("$n says 'I hereby declare the city of Tokyo open!'",
	    ch, NULL, NULL, TO_ROOM);
	break;

    case 'E':
	act("$n says 'I hereby declare the city of Tokyo closed!'",
	    ch, NULL, NULL, TO_ROOM);
	break;
...
bool spec_cabbit(CHAR_DATA * ch)
{
    ...
    /* Cabbit logic:  Happiness == carrots! ^_^ */

    /* Punt if sleeping, or blind. */
    if (!IS_AWAKE(ch) || IS_AFFECTED(ch, AFF_BLIND)) {
	return (FALSE);
    }

    /* Punt on paranoia */
    if (ch->in_room == NULL || ch->in_room->people == NULL) {
	return (FALSE);
    }

    /* Do I have a carrot? */
    if ((carrot_obj = get_obj_carry(ch, "carrot")) == NULL) {
	/* No. Does anyone around have one? */
	for (vch = ch->in_room->people; vch != NULL;
	     vch = vch->next_in_room) {

	    /* Don't bother with NPCs, they won't give me one anyway. */
	    if (IS_NPC(vch)) {
		continue;
	    }
	    ...
	    /* Don't do people I can't see */
	    if (!can_see(ch, vch)) {
		continue;
	    }

	    /* Do they have a carrot? */
	    if ((carrot_obj = get_obj_carry(vch, "carrot")) == NULL) {
		/* No. */
		continue;
	    } else {
		/* Edible carrot? */
		if (carrot_obj->item_type != ITEM_FOOD) {
		    /* No, darn. */
		    continue;
		} else {
		    /* Yes!  Beg, maybe they'll give it to me! */
		    send_to_char("Waiwaiwai!  Carrot!\n\r", ch);
		    switch (number_range(1, 5)) {
		    case 1:
			sprintf(message1,
				"$n runs in estatic circles around $N's feet!");
			sprintf(message2,
				"$n runs in estatic circles around your feet!");
			break;
		    case 2:
			sprintf(message1,
				"$n sits up on $s hind legs and begs $N for a carrot!");
			sprintf(message2,
				"$n sits up on $s hind legs and begs you for a carrot!");
			break;
		    case 3:
			sprintf(message1,
				"$n looks adoringly at $N, what a terrible beggar!");
			sprintf(message2,
				"$n looks adoringly at you, what a terrible beggar!");
			break;
		    case 4:
			sprintf(message1,
				"$n stares at $N with $s big cabbit eyes!");
			sprintf(message2,
				"$n stares at you with $s big cabbit eyes!");
			break;
		    case 5:
			sprintf(message1,
				"$n looks at $N and niaows excitedly!");
			sprintf(message2,
				"$n looks at you and niaows excitedly!");
			break;
		    default:
			sprintf(message1,
				"$n sits up on $s hind legs and niaows loudly!");
			sprintf(message2,
				"$n sits up on $s hind legs and niaows loudly!");
			break;
		    }
		    act(message1, ch, NULL, vch, TO_NOTVICT);
		    act(message2, ch, NULL, vch, TO_VICT);
		    return (FALSE);
		}
	    }
	}
	...
	if (carrot_obj->item_type != ITEM_FOOD) {
	    /* No... People are mean! */
	    act("$n looks strangely at $p.", ch, carrot_obj, NULL,
		TO_ROOM);
	    act("$p isn't edible, darn!", ch, carrot_obj, NULL, TO_CHAR);
	    act("$n drops $p.", ch, carrot_obj, NULL, TO_ROOM);
	    obj_from_char(carrot_obj);
	    obj_to_room(carrot_obj, ch->in_room);
	    return (FALSE);
	} else {
	    /* Yes, happiness! */
	    act("$n takes $p in $s paws, with a look of joy in $s eyes!",
		ch, carrot_obj, NULL, TO_ROOM);
	    send_to_char("Carrot, Carrot!  Joy of joys!\n\r", ch);
	    do_eat(ch, "carrot");
	    return (TRUE);
	}
	/* Can't get here */
