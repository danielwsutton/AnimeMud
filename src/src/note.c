...
int count_two_spool(CHAR_DATA * ch, NOTE_DATA * spool)
{
    int count_two = 0;
    NOTE_DATA *pnote;
    for (pnote = spool; pnote != NULL; pnote = pnote->next)
...
int count_three_spool(CHAR_DATA * ch, NOTE_DATA * spool)
{
    int count_three = 0;
    NOTE_DATA *pnote;
    for (pnote = spool; pnote != NULL; pnote = pnote->next)
...
void do_unread(CHAR_DATA * ch)
{
    ...
    send_to_char("   `&A``n`&i``m`&e M``U`&D`` Notes Bulletin.\n\r", ch);
    send_to_char("`A-``--`&--------------------------``--`A-``\n\r", ch);
    send_to_char("`&[`!T`1tl`&] [`$N`4ew`&] [`@Y`2ou`&]``\n\r", ch);
    if ((count = count_spool(ch, news_list)) >= 0) {
	count_two = count_two_spool(ch, news_list);
	count_three = count_three_spool(ch, news_list);
	sprintf(buf, "[%3d] [%3d] [%3d] : News articles.\n\r",
		count_two, count, count_three);
	send_to_char(buf, ch);
    } else
	send_to_char("[  0] [  0] : News articles.\n\r", ch);
    if ((count = count_spool(ch, changes_list)) >= 0) {
	count_two = count_two_spool(ch, changes_list);
	count_three = count_three_spool(ch, changes_list);
	sprintf(buf, "[%3d] [%3d] [%3d] : Changes.\n\r", count_two, count,
		count_three);
	send_to_char(buf, ch);
    } else
	send_to_char("[  0] [  0] : Changes.\n\r", ch);
    if ((count = count_spool(ch, note_list)) >= 0) {
	count_two = count_two_spool(ch, note_list);
	count_three = count_three_spool(ch, note_list);
	sprintf(buf, "[%3d] [%3d] [%3d] : Notes.\n\r",
		count_two, count, count_three);
	send_to_char(buf, ch);
    } else
	send_to_char("[  0] [  0] : Notes.\n\r", ch);
    if ((count = count_spool(ch, idea_list)) >= 0) {
	count_two = count_two_spool(ch, idea_list);
	count_three = count_three_spool(ch, idea_list);
	sprintf(buf, "[%3d] [%3d] [%3d] : Ideas.\n\r",
		count_two, count, count_three);
	send_to_char(buf, ch);
    } else
	send_to_char("[  0] [  0] : Ideas.\n\r", ch);
    if ((count = count_spool(ch, bounty_list)) >= 0) {
	count_two = count_two_spool(ch, bounty_list);
	count_three = count_three_spool(ch, bounty_list);
	sprintf(buf, "[%3d] [%3d] [%3d] : RPnote.\n\r",
		count_two, count, count_three);
	send_to_char(buf, ch);
    } else
	send_to_char("[  0] [  0] : RPnote.\n\r", ch);
    if (IS_TRUSTED(ch, ANGEL)) {
	if ((count = count_spool(ch, penalty_list)) >= 0) {
	    count_two = count_two_spool(ch, penalty_list);
	    count_three = count_three_spool(ch, penalty_list);
	    sprintf(buf, "[%3d] [%3d] [%3d] : Penalties.\n\r",
		    count_two, count, count_three);
	    send_to_char(buf, ch);
	} else
	    send_to_char("[  0] [  0] : Penalties.\n\r", ch);
    }
    send_to_char("`A-``--`&--------------------------``--`A-``\n\r", ch);
}
...
{
    parse_note(ch, argument, NOTE_BOUNTY);
}
...
void load_notes(void)
{
    ...
    load_thread(BOUNTY_FILE, &bounty_list, NOTE_BOUNTY, 0);
}
...
bool is_note_to(CHAR_DATA * ch, NOTE_DATA * pnote)
{
    ...
    if (IS_WIZARD(ch) && is_name("wiz", pnote->to_list))
	return TRUE;

    if (IS_CODER(ch) && is_name("coders", pnote->to_list))
	return TRUE;
...
/* Hacked for new clans - Suzuran
	...
	return TRUE;
*/
    if (ch->clan && is_name(get_clan_name(ch->clan), pnote->to_list))
	return TRUE;
...
bool hide_note(CHAR_DATA * ch, NOTE_DATA * pnote)
{
	...
	break;
    case NOTE_BOUNTY:
	last_read = ch->pcdata->last_bounty;
	break;
    }
...
void update_read(CHAR_DATA * ch, NOTE_DATA * pnote)
{
	...
	break;
    case NOTE_BOUNTY:
	ch->pcdata->last_bounty = UMAX(ch->pcdata->last_bounty, stamp);
	break;
    }
}
...
{
    char buf[MAX_STRING_LENGTH],
	arg[MAX_INPUT_LENGTH], *list_name, *list_name_s;
    int vnum, anum;
    BUFFER *buffer;
    CHAR_DATA *wch;
	...
	if (!str_cmp(argument, "all")) {
	    fAll = TRUE;
	    anum = 0;
	} else if (argument[0] == '\0' || !str_prefix(argument, "next")) {	/* read next unread note */
	    vnum = 0;
	...
	if (buffer) {		// we found a matching note and need to otuput it
	    page_to_char(buf_string(buffer), ch);
	    free_buf(buffer);
	    WAIT_STATE(ch, (1.5 * PULSE_VIOLENCE));
	    return;
	}
	// We found no matching notes
	...
	for (pnote = *list; pnote != NULL; pnote = pnote->next) {
	    if (is_note_to(ch, pnote)) {
		sprintf(buf, "[%3d%s]`4From``: %10s `3Subject``: %s\n\r",
	...
	case NOTE_CHANGES:
	    ...
	    break;
	case NOTE_BOUNTY:
	    ch->pcdata->last_bounty = current_time;
	    break;
	}
	return;
    }
    ...
    if ((type == NOTE_NEWS && !IS_TRUSTED(ch, ADMIN))
    ...
    if (!str_cmp(arg, "+")) {
	if (IS_VALIDATION(ch)) {
	    send_to_char("Wait till you're validated to write notes.\n\r",
			 ch);
	    return;
	}
	...
	if (IS_SET(ch->comm, COMM_NOCHANNELS)) {
	    send_to_char("You can't post notes when nochanneled.\n\r", ch);
	    return;
	}
	...
	if (!str_cmp(ch->pnote->to_list, "")) {
	    send_to_char
		("You need to provide a recipient (name, all, immortal, immort, imp or wiz).\n\r",
		 ch);
	    return;
	}
	...
	for (wch = char_list; wch != NULL; wch = wch->next) {
	    if (is_note_to(wch, ch->pnote) && ch != wch) {
		sprintf(buf, "[`&INFO``]: You have a new %s`&.``\n\r",
			list_name_s);
