...
void do_unread(CHAR_DATA * ch)
{
    ...
    send_to_char("   Prophecy Notes Bulletin.\n\r", ch);
    send_to_char("`4--------------------------------``\n\r", ch);
    send_to_char("`&[`1Ttl`&] [`4New`&] ``\n\r", ch);
    if ((count = count_spool(ch, news_list)) >= 0) {
	count_two = count_two_spool(ch, news_list) + 1;
	sprintf(buf, "[%3d] [%3d] : News articles.\n\r", count_two, count);
	send_to_char(buf, ch);
    } else
	send_to_char("[  0] [  0] : News articles.\n\r", ch);
    if ((count = count_spool(ch, changes_list)) >= 0) {
	count_two = count_two_spool(ch, changes_list) + 1;
	sprintf(buf, "[%3d] [%3d] : Changes.\n\r", count_two, count);
	send_to_char(buf, ch);
    } else
	send_to_char("[  0] [  0] : Changes.\n\r", ch);
    if ((count = count_spool(ch, note_list)) >= 0) {
	count_two = count_two_spool(ch, note_list) + 1;
	sprintf(buf, "[%3d] [%3d] : Notes.\n\r", count_two, count);
	send_to_char(buf, ch);
    } else
	send_to_char("[  0] [  0] : Notes.\n\r", ch);
    if ((count = count_spool(ch, idea_list)) >= 0) {
	count_two = count_two_spool(ch, idea_list) + 1;
	sprintf(buf, "[%3d] [%3d] : Ideas.\n\r", count_two, count);
	send_to_char(buf, ch);
    } else
	send_to_char("[  0] [  0] : Ideas.\n\r", ch);
    if ((count = count_spool(ch, bounty_list)) >= 0) {
	count_two = count_two_spool(ch, bounty_list) + 1;
	sprintf(buf, "[%3d] [%3d] : Bounties.\n\r", count_two, count);
	send_to_char(buf, ch);
    } else
	send_to_char("[  0] [  0] : Bounties.\n\r", ch);
    if (IS_TRUSTED(ch, ANGEL)) {
	if ((count = count_spool(ch, penalty_list)) >= 0) {
	    count_two = count_spool(ch, penalty_list) + 1;
	    sprintf(buf, "[%3d] [%3d] : Penalties.\n\r", count_two, count);
	    send_to_char(buf, ch);
	} else
...
void parse_note(CHAR_DATA * ch, char *argument, int type)
{
	...
	for (pnote = *list; pnote != NULL; pnote = pnote->next) {
	    if (is_note_to(ch, pnote)) {
		sprintf(buf, "[%3d%s]`4From``: %s `3Subject``: %s\n\r",
    ...
    if ((type == NOTE_NEWS && !IS_TRUSTED(ch, WIZARD))
