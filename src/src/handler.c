...
/* EXTERNS FOR NEW CLAN STUFF */
extern sh_int get_clan_hall(CHAR_DATA * ch);
extern sh_int get_clan_droom(CHAR_DATA * ch);

/*
 * Local functions.
 */
void affect_modify args((CHAR_DATA * ch, AFFECT_DATA * paf, bool fAdd));
void affect_strip_perm args((CHAR_DATA * ch, int sn));	/*EE960521 */
void affect_remove_perm args((CHAR_DATA * ch, AFFECT_DATA * paf));	/*EE960521 */

void nuke_quest args((CHAR_DATA * ch));
...
int get_skill(CHAR_DATA * ch, int sn)
{
	...
	if (skill_table[sn].spell_fun != spell_null)
	    ...
	    skill = 4 * ch->level - 65;
	else if (sn == gsn_senses && (IS_SET(ch->act, ACT_WARRIOR)))
	    skill = 4 * ch->level - 40;
	else if (sn == gsn_possess && (IS_SET(ch->act, ACT_THIEF)))
	    skill = 4 * ch->level - 40;
	else if (sn == gsn_hand_to_hand)
	    skill = 40 + 2 * ch->level;
	else if (sn == gsn_trip && IS_SET(ch->off_flags, OFF_TRIP))
	    skill = 10 + 3 * ch->level;
	else if (sn == gsn_dragonp && IS_SET(ch->off_flags, OFF_TRIP))
	    skill = 10 + 3 * ch->level;
	else if (sn == gsn_hurricane && IS_SET(ch->off_flags, OFF_KICK))
	    skill = 10 + 3 * ch->level;
	else if (sn == gsn_roundhouse && IS_SET(ch->off_flags, OFF_KICK))
	    skill = 10 + 3 * ch->level;
	else if (sn == gsn_flashkick && IS_SET(ch->off_flags, OFF_KICK))
	    skill = 10 + 3 * ch->level;
	else if (sn == gsn_optic && IS_SET(ch->off_flags, OFF_KICK))
	    skill = 10 + 3 * ch->level;
	else if (sn == gsn_feed && IS_SET(ch->off_flags, OFF_KICK))
	    skill = 10 + 3 * ch->level;
	else if (sn == gsn_throw && IS_SET(ch->off_flags, OFF_KICK))
	    skill = 10 + 3 * ch->level;
	else if (sn == gsn_footsweep && IS_SET(ch->off_flags, OFF_KICK))
	    skill = 10 + 3 * ch->level;
	    ...
	    skill = 3 * ch->level;
	else if (sn == gsn_battle_rage
		 && IS_SET(ch->off_flags, OFF_BERSERK)) skill =
		3 * ch->level;
	else if (sn == gsn_kick)
	    skill = 10 + 3 * ch->level;
...
int get_stat_mod(CHAR_DATA * ch, int stat)
{
    int mod;

    mod = get_curr_stat(ch, stat);
...
int can_carry_w(CHAR_DATA * ch)
{
    ...
    if ((!IS_NPC(ch) && ch->level >= LEVEL_IMMORTAL) ||
	(IS_NPC(ch) && IS_SET(ch->act, ACT_IS_BANKER)))
	/*EE960808 */
	return 10000000;
...
void affect_modify(CHAR_DATA * ch, AFFECT_DATA * paf, bool fAdd)
{
    ...
    case APPLY_DETECT_MAGIC:
	{
		...
		affect_strip(ch, af.type);
		send_to_char("You can no longer sense magical auras.\n\r",
			     ch);
		break;
	    }
	}
	...
	/*
	   ...
	   act("$n is protected from evil.", ch, NULL, NULL, TO_ROOM);
...
void affect_to_char(CHAR_DATA * ch, AFFECT_DATA * paf)
{
    ...
    *paf_new = *paf;
    VALIDATE(paf_new);		/* Be paranoid! -- Suzuran */
    ...
    if (paf_new->location == APPLY_SANCTUARY) {

	paf_new->location = APPLY_NONE;

	affect_modify(ch, paf_new, TRUE);

	paf_new->location = APPLY_SANCTUARY;
...
/* OLD VERSION (before perm) void affect_to_obj(OBJ_DATA *obj, AFFECT_DATA *paf)
...
void affect_remove(CHAR_DATA * ch, AFFECT_DATA * paf)
{
    ...
    /* Handle Chibichibification */
    if(is_affected(ch,skill_lookup("chibichibification"))){
      char buf[MAX_STRING_LENGTH];

      /* Yes.  Swap descriptions back */
      // printf("Removing Chibi: desc %s, saved %s\n",ch->description,ch->savedesc);
      if(ch->savedesc != NULL){
        free_string(ch->description);
        sprintf(buf,"%s",ch->savedesc);
        ch->description = str_dup(buf);
        free_string(ch->savedesc);
        /* BUG - FREE_STRING() DOES *NOT* MAKE THE POINTER
           NULL, IT LEAVES IT POINTING AT INVALID MEMORY! */
        ch->savedesc = NULL;
      }
      /* Seize chibichibi eq if they have it */
      {
        OBJ_DATA *obj;

        obj=get_eq_char(ch,WEAR_BODY);
        if(obj != NULL && obj->pIndexData != NULL && obj->pIndexData->vnum == OBJ_VNUM_SCHOOL_VEST){
          unequip_char(ch,obj); obj_from_char(obj);
          extract_obj(obj);
        }
        obj=get_eq_char(ch,WEAR_FEET);
        if(obj != NULL && obj->pIndexData != NULL && obj->pIndexData->vnum == 3708){
          unequip_char(ch,obj); obj_from_char(obj);
          extract_obj(obj);
        }
        obj=get_eq_char(ch,WEAR_LEGS);
        if(obj != NULL && obj->pIndexData != NULL && obj->pIndexData->vnum == 3707){
          unequip_char(ch,obj); obj_from_char(obj);
          extract_obj(obj);
        }
        obj=get_eq_char(ch,WEAR_HANDS);
        if(obj != NULL && obj->pIndexData != NULL && obj->pIndexData->vnum == 3709){
          unequip_char(ch,obj); obj_from_char(obj);
          extract_obj(obj);
        }
        obj=get_eq_char(ch,WEAR_NECK_1);
        if(obj != NULL && obj->pIndexData != NULL && obj->pIndexData->vnum == 3705){
          unequip_char(ch,obj); obj_from_char(obj);
          extract_obj(obj);
        }
      }
	...
	if (obj_to->carried_by != NULL) {
		...
		* WEIGHT_MULT(obj_to) / 100;
		...
		obj_to->carried_by->carry_weight +=
		    get_obj_weight(obj) * WEIGHT_MULT_TWO(obj_to) / 100;
...
/*        char_to_room(ch,get_room_index(clan_table[ch->clan].hall));
		return; */
	/* CHANGE FOR NEW CLAN STUFF */
	char_to_room(ch, get_room_index(get_clan_droom(ch)));
	return;
...
/* Count name matches - Suzuran */

int count_obj_match(CHAR_DATA * ch, char *argument, OBJ_DATA * list)
{
    OBJ_DATA *obj;
...
int count_carry_match(CHAR_DATA * ch, char *argument)
{
    char arg[MAX_INPUT_LENGTH];
    OBJ_DATA *obj;
    int number = number_argument(argument, arg), count = 0;
...
OBJ_DATA *get_obj_here(CHAR_DATA * ch, char *argument)
{
    ...
    /* Have to frob object numbers to prevent certain types of
       stupidity WRT multiple copies of objects in different places.
       -- Suzuran */

    count = number_argument(argument, buffer2);

    obj = get_obj_list(ch, argument, ch->in_room->contents);
    if (obj != NULL)
	return obj;

    /* Here's how to not screw up these numbers.
       First, get count, and the buffer... */
    count = number_argument(argument, buffer);
    /* Drop the count by the number of objects in the previous
       searched area. (count_obj_list doesn't do here because we're
       counting names, not actual objects ) */
    count = count - count_obj_match(ch, buffer, ch->in_room->contents);
    /* Put argument back together if I have to */
    if (count != 0) {
	sprintf(buffer, "%d.%s", count, buffer2);
    };

    if ((obj = get_obj_carry(ch, buffer)) != NULL)
	return obj;

    count = number_argument(buffer, buffer);
    count = count - count_carry_match(ch, buffer);
    if (count != 0) {
	sprintf(buffer, "%d.%s", count, buffer2);
    };

    if ((obj = get_obj_wear(ch, buffer)) != NULL)
	return obj;

    /* Not even going to bother for horses */

    if (ch->horse != NULL && ch->horse->in_room == ch->in_room)
	if ((obj = get_obj_wear(ch->horse, buffer)) != NULL)
	    return obj;

    return NULL;
}

/* SPECIAL VERSION for put commands, search inventory first */
OBJ_DATA *get_obj_here_invfirst(CHAR_DATA * ch, char *argument)
{
    OBJ_DATA *obj;
    ...
    obj = get_obj_list(ch, buffer, ch->in_room->contents);
    if (obj != NULL)
	return obj;

    if (ch->horse != NULL && ch->horse->in_room == ch->in_room)
	if ((obj = get_obj_wear(ch->horse, argument)) != NULL)
	    return obj;
...
bool room_is_private(ROOM_INDEX_DATA * pRoomIndex)
{
    ...
    if (IS_SET(pRoomIndex->room_flags, ROOM_WIZARD_ONLY))
	return TRUE;
...
bool can_see_room(CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex)
{
	...
	&& ch->clan != pRoomIndex->clan) return FALSE;
...
/*TG970914
 *Added so clan members could see other clanmembers who are
 *not incog or invis automatically w/clan who
 */

/* USE NEW VERSION IN NEWCLANS.C
...
bool can_see_obj(CHAR_DATA * ch, OBJ_DATA * obj)
{
    ...
    /* Items on invisible people should be invisible too - Suzuran */
...
char *affect_loc_name(int location)
{
    ...
    case APPLY_FRENZY:
	return "frenzy";
    case APPLY_STONE_SKIN:
	return "stone_skin";
    case APPLY_GIANT_STR:
	return "giant_str";
    case APPLY_BLESS:
	return "bless";
    case APPLY_ARMOR:
...
/*
 *EE960518
 * Return affect location from a ascii name.
 */
int affect_location(char *argument)
{
    if (!strcmp(argument, "none"))
	return APPLY_NONE;
    else if (!strcmp(argument, "strength"))
	return APPLY_STR;
    else if (!strcmp(argument, "dexterity"))
	return APPLY_DEX;
    else if (!strcmp(argument, "intelligence"))
	return APPLY_INT;
    else if (!strcmp(argument, "wisdom"))
	return APPLY_WIS;
    else if (!strcmp(argument, "constitution"))
	return APPLY_CON;
    else if (!strcmp(argument, "sex"))
	return APPLY_SEX;
    else if (!strcmp(argument, "class"))
	return APPLY_CLASS;
    else if (!strcmp(argument, "level"))
	return APPLY_LEVEL;
    else if (!strcmp(argument, "age"))
	return APPLY_AGE;
    else if (!strcmp(argument, "mana"))
	return APPLY_MANA;
    else if (!strcmp(argument, "hp"))
	return APPLY_HIT;
    else if (!strcmp(argument, "moves"))
	return APPLY_MOVE;
    else if (!strcmp(argument, "gold"))
	return APPLY_GOLD;
    else if (!strcmp(argument, "experience"))
	return APPLY_EXP;
    else if (!strcmp(argument, "armor class"))
	return APPLY_AC;
    else if (!strcmp(argument, "hitroll"))
	return APPLY_HITROLL;
    else if (!strcmp(argument, "damage roll"))
	return APPLY_DAMROLL;
    else if (!strcmp(argument, "dam roll"))
	return APPLY_DAMROLL;
    else if (!strcmp(argument, "saves"))
	return APPLY_SAVES;
    else if (!strcmp(argument, "save vs rod"))
	return APPLY_SAVING_ROD;
    else if (!strcmp(argument, "save vs petrification"))
	return APPLY_SAVING_PETRI;
    else if (!strcmp(argument, "save vs breath"))
	return APPLY_SAVING_BREATH;
    else if (!strcmp(argument, "save vs spell"))
	return APPLY_SAVING_SPELL;
    else if (!strcmp(argument, "sanctuary"))
	return APPLY_SANCTUARY;
    else if (!strcmp(argument, "sanc"))
	return APPLY_SANCTUARY;
...
char *extra_bit_name(int extra_flags)
{
    ...
    if (extra_flags & ITEM_GUNBURST)
	strcat(buf, " gunburst");
    if (extra_flags & ITEM_BURN_PROOF)
	strcat(buf, " burn_proof");
    if (extra_flags & ITEM_NOUNCURSE)
	strcat(buf, " no_uncurse");
    if (extra_flags & ITEM_NOAUCTION)
	strcat(buf, " no_auction");
    if (extra_flags & ITEM_CLAN_EQ)
	strcat(buf, " clan_eq");
    if (extra_flags & ITEM_QUEST_EQ)
	strcat(buf, " quest_eq");
    if (extra_flags & ITEM_AUTOQUEST_EQ)
	strcat(buf, " autoquest_eq");
    if (extra_flags & ITEM_GUNAUTO)
	strcat(buf, " gunauto");
...
char *act_bit_name(int act_flags)
{
	...
	if (act_flags & PLR_AUTODAMAGE)
	    strcat(buf, " autodamage");
...
char *imm_bit_name(int imm_flags)
{
	...
	strcat(buf, " wood");
    if (imm_flags & VULN_CUENDILLAR)
	strcat(buf, "cuendillar");
...
char *room_bit_name(int room_flags)
{
	...
	strcat(buf, " god only");
    if (room_flags & ROOM_WIZARD_ONLY)
	strcat(buf, " wizard only");
    if (room_flags & ROOM_HEROES_ONLY)
	strcat(buf, " hero only");
    if (room_flags & ROOM_NEWBIES_ONLY)
	strcat(buf, " newbies only");
    if (room_flags & ROOM_LAW)
	strcat(buf, " law");
    if (room_flags & ROOM_NOWHERE)
	strcat(buf, " nowhere");
    if (room_flags & ROOM_CLAN_ONLY)
	strcat(buf, " clan_only");
    if (room_flags & ROOM_CLAN_SHOP)
	strcat(buf, " clan_shop");
/*	if (room_flags & ROOM_108			) strcat(buf, " 108_clan_hall"			);
	if (room_flags & ROOM_JUDGE			) strcat(buf, " Judge_clan_hall"		);
	if (room_flags & ROOM_MDS			) strcat(buf, " MDS_clan_hall"			);
	if (room_flags & ROOM_SAILOR		) strcat(buf, " Sailor_clan_hall"		);
	if (room_flags & ROOM_VAMPIRE		) strcat(buf, " VampireHunt_clan_hall"	);
	if (room_flags & ROOM_INVID			) strcat(buf, " Invid_clan_hall"		);
	if (room_flags & ROOM_SHINOBI		) strcat(buf, " Shinobi_clan_hall"		);
	if (room_flags & ROOM_SHOGUN		) strcat(buf, " Shogun_clan_hall"		);
	if (room_flags & ROOM_HUNTER_WARRIOR) strcat(buf, " H-Warrior_clan_hall"	);
	*/
