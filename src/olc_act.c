...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
	...
	case ITEM_HEAVYGUN:
	case ITEM_ENERGYGUN:
	    if (obj->value[0] < BULLET_CALIBER) {
		sprintf( buf, "[v0] Caliber: %s [%ld]\n\r",
		    ammo_table[obj->value[0]].caliber, obj->value[0] );
		send_to_char( buf, ch );
		sprintf( buf, "[v1] Ammotype: %s [%ld]\n\r",
		    obj->value[1] < BULLET_TYPE ?
		    ammo_table[obj->value[0]].ammotype[obj->value[1]] : "(null)", obj->value[1] );
		send_to_char( buf, ch );
	    } else {
		sprintf( buf, "[v0] Caliber: (null) [%ld]\n\r", obj->value[0] );
		send_to_char( buf, ch );
		sprintf( buf, "[v1] Ammotype: (null) [%ld]\n\r", obj->value[1] );
		send_to_char( buf, ch);
	    }

	    sprintf( buf, "[v2] Ammunition: %ld\n\r", obj->value[2] );
	    send_to_char( buf, ch );
	    sprintf( buf, "[v3] Damage modifier: %ld\n\r", obj->value[3] );
	    send_to_char( buf, ch );
	    sprintf( buf, "[v4] Max ammo capacity: %ld\n\r", obj->value[4] );
	    send_to_char( buf, ch );
	    break;
	...
	case ITEM_CLIP:
	    ...
	    sprintf( buf, "[v2] Loaded rounds: %ld\n\r", obj->value[2] );
	    send_to_char( buf, ch );
	    sprintf( buf, "[v3] Max capacity: %ld\n\r", obj->value[3] );
	    send_to_char( buf, ch );
	    sprintf( buf, "[v4] Loads into: %s [vnum %ld]\n\r",
		get_obj_index(obj->value[4]) != NULL ?
		(IS_GUN(get_obj_index(obj->value[4])) ?
		get_obj_index(obj->value[4])->short_descr : "(null)") : "(null)", obj->value[4] );
	    send_to_char( buf, ch );
	    break;
    }
...
/* OLC		    do_help( ch, "liquids" );    */
	...
	case ITEM_ENERGYGUN:
		...
		default:
		    ...
		    return FALSE;
		case 0:
		    send_to_char( "GUN CALIBER SET.\n\r\n\r", ch );
		    pObj->value[0] = atoi( argument );
		    break;
		case 1:
		    send_to_char( "LOADED AMMUNITION TYPE SET.\n\r\n\r", ch );
		    pObj->value[1] = atoi( argument );
		    break;
		case 2:
		    send_to_char( "LOADED AMMUNITION AMOUNT SET.\n\r\n\r", ch );
		    pObj->value[2] = atoi( argument );
		    break;
		case 3:
		    send_to_char( "SHOT DAMAGE MODIFIER SET.\n\r\n\r", ch );
		    pObj->value[3] = atoi( argument );
		    break;
		case 4:
		    send_to_char( "MAX AMMO CAPACITY SET.\n\r\n\r", ch );
		    pObj->value[4] = atoi( argument );
		    break;
	    }
	    break;
	...
	case ITEM_AMMO:
		...
		default:
		    ...
		    return FALSE;
		case 0:
		    send_to_char( "AMMUNITION CALIBER SET.\n\r\n\r", ch );
		    pObj->value[0] = atoi( argument );
		    break;
		case 1:
		    send_to_char( "AMMUNITION TYPE SET.\n\r\n\r", ch );
		    pObj->value[1] = atoi( argument );
		    break;
		case 2:
		    send_to_char( "AMMUNITION AMOUNT SET.\n\r\n\r", ch );
		    pObj->value[2] = atoi( argument );
		    break;
	    }
	    break;
	...
	case ITEM_CLIP:
	    ...
	    {
		default:
		    do_help( ch, "ITEM_CLIP" );
		    return FALSE;
		case 0:
		    send_to_char( "MAGAZINE CALIBER SET.\n\r\n\r", ch );
		    pObj->value[0] = atoi( argument );
		    break;
		case 1:
		    send_to_char( "LOADED AMMUNITION TYPE SET.\n\r\n\r", ch );
		...
		case 2:
		    ...
		    break;
		case 3:
		    send_to_char( "MAGAZINE MAX CAPACITY SET.\n\r\n\r", ch );
		    pObj->value[3] = atoi( argument );
		    break;
		case 4:
		    send_to_char( "GUN COMPATIBILITY VNUM SET.\n\r\n\r", ch );
		    pObj->value[4] = atoi( argument );
		    break;
	    }
	    break;
